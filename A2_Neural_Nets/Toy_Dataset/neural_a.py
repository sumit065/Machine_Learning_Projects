...
#read params: 5 lines? 1.learning strategy, 2.learning rate 3. max iter , 4.batch size, 5. hidden layer arch
params_arr = []
...
def backprop_wt_update(ds_dict, activations_dict, weights_dict, y_pred, y_train, x_train, eta):
    ...
    while i>1:
        if i == len(weights_dict):
            cur_ds = (y_pred - y_train)/float(train_size)
        else:
            cur_ds = ds_dict[i]
...
Y_train = np.array([df[len(df.columns)-1]]).T
